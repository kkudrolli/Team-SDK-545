...
int main(int argc, char *argv[])
{
  ...
  if (option == 'i') { // Option for camera image
	...
	return 0;
...
#ifdef TRANSFER
      transfer(m_imgs->imgs, m_lbls->labels->data, port, 1, num_imgs);  // Changed to train!!
#endif
    } else if (strncmp(train_test, "clobber", 7) == 0) { // train
      mnist_images_t m_imgs = read_images(TRAIN);
      mnist_labels_t m_lbls = read_labels(TRAIN);
      ...
      for (int i = 0; i < m_lbls->labels->length; i++) {
	m_lbls->labels->data[i] = rand() % 10;
      }
