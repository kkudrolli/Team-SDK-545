module tile
  (clk, rst, start_fp, start_bp, done, get_weights0, get_weights1, image, 
   ...
   enum 				 logic [3:0] {S_IDLE, S_FPROP_LAYER1, S_FPROP_LAYER2, 
						      ...
						      S_DONE, S_DELAY1, S_DELAY2, S_DELAY3, S_DELAY4,
						      S_DELAY5} cs, ns;
   ...
   generate
      for (i = 0; i < NUM_NEURONS; i++) begin
         neuron layer0 (.clk(clk), .rst(rst), .clear(clear), .en (enable_0&&(lay0_idx<IMG_SZ)), .weight (weights0[NUM_NEURONS-i-1]), 
      ...
      for (i = 0; i < OUTPUT_SZ; i++) begin
         neuron layer1 (.clk(clk), .rst(rst), .clear(clear), .en (enable_1&&(lay1_idx<NUM_NEURONS)), .weight (weights1[OUTPUT_SZ-i-1]),
   ...
   generate
      for (i = 0; i < NUM_NEURONS; i++) begin
         ...
         multiplier_div32 mult2(delta_drv[i], image_reg[lay0_idx], wchange0[NUM_NEURONS-i-1]);
      end
      for (i = 0; i < OUTPUT_SZ; i++) begin
         multiplier_div32 mult3(delta_target[i], hidden[lay1_idx], wchange1[OUTPUT_SZ-i-1]);
         multiplier mult4(weights1[OUTPUT_SZ-i-1], delta_target[i], mult[i]);
      end
   endgenerate
   ...
   always_comb begin
      ...
      case (cs)
        ...
        S_FPROP_LAYER1: begin
         ns = (lay0_idx < IMG_SZ+2) ? S_FPROP_LAYER1 : S_DELAY2;
        ...
        S_FPROP_LAYER2: begin
           enable_1 = 1;
           ns = lay1_idx < NUM_NEURONS+2 ? S_FPROP_LAYER2 : (bp ? S_DELAY3 : S_DONE);
           update1 = bp && (lay1_idx >= NUM_NEURONS+2);
        end
    
        S_DELAY3: ns = S_BPROP_LAYER2;
